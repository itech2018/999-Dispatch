<?php
/*
9/10/13 - requests.php - lists pending / open requests for Service user
*/
...
<html>
<HEAD>
<TITLE>Tickets - Service User Requests</TITLE>
...
function get_requests() {
	...
	function requests_cb(req) {
		...
		if(the_requests[0] == "No Current Requests") {
			...
			width = "width: 100%; ";
			the_string = "<TABLE id='reqstable' cellspacing='0' cellpadding='1' style='width: 100%; color: #FFFFFF;'>";
			the_string += "<TR class='heading text' style='" + width + "font-weight: bold;'>";
			the_string += "<TD class='heading text' style='font-weight: bold; border-right: 1px solid #FFFFFF;'><?php print get_text('ID');?></TD>";
			the_string += "<TD class='heading text' style='font-weight: bold; border-right: 1px solid #FFFFFF;'><?php print get_text('Danger');?></TD>";
			...
			the_string += "<TD class='heading text' style='font-weight: bold; border-right: 1px solid #FFFFFF;'><?php print get_text('Scope');?></TD>";
			the_string += "<TD class='heading text' style='font-weight: bold; border-right: 1px solid #FFFFFF;'><?php print get_text('Description');?></TD>";
			...
			for(var key in the_requests) {
				...
				var isUrgent = the_requests[key][36];
				var urgentText = (isUrgent) ? "YES" : "";
				if(isUrgent) {
					the_string += "<TR title='" + theTitle + "' style='background-color: red; color: black; border-bottom: 2px solid #000000; height: 30px;'>";
					} else {
					the_string += "<TR title='" + theTitle + "' style='" + the_requests[key][17] + "; border-bottom: 2px solid #000000; height: 30px;'>";
					}
				the_string += "<TD style='white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;' onClick=\"window.open('request.php?id=" + the_request_id + "','view_request','width=600,height=600,titlebar=1, location=0, resizable=1, scrollbars=yes, status=0,toolbar=0,menubar=0,location=0, right=100,top=300,screenX=500,screenY=300')\">" + the_requests[key][0] + "</TD>";
				the_string += "<TD style='text-align: center; white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;' onClick=\"window.open('request.php?id=" + the_request_id + "','view_request','width=600,height=600,titlebar=1, location=0, resizable=1, scrollbars=yes, status=0,toolbar=0,menubar=0,location=0, right=100,top=300,screenX=500,screenY=300')\">" + urgentText + "</TD>";
				the_string += "<TD style='white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;' onClick=\"window.open('request.php?id=" + the_request_id + "','view_message','width=600,height=600,titlebar=1, location=0, resizable=1, scrollbars=yes, status=0,toolbar=0,menubar=0,location=0, right=100,top=300,screenX=500,screenY=300')\">" + the_requests[key][2] + "</TD>";
				...
				the_string += "<TD style='width: 15%; white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;' onClick=\"window.open('request.php?id=" + the_request_id + "','view_message','width=600,height=600,titlebar=1, location=0, resizable=1, scrollbars=yes, status=0,toolbar=0,menubar=0,location=0, right=100,top=300,screenX=500,screenY=300')\">" + theField + "</TD>";
				the_string += "<TD title='" + the_requests[key][14] + "' style='white-space: normal; word-wrap: break-word; -ms-word-wrap: sWrap; border-right: 1px solid #707070;' onClick=\"window.open('request.php?id=" + the_request_id + "','view_message','width=600,height=600,titlebar=1, location=0, resizable=1, scrollbars=yes, status=0,toolbar=0,menubar=0,location=0, right=100,top=300,screenX=500,screenY=300')\">" + the_requests[key][14].substring(0,48)+"..." + "</TD>";
				the_string += "<TD style='white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;' " + the_onclick + ">" + the_requests[key][16] + "</TD>";	
				...
				if(the_requests[key][35] != 0) {
					the_string += "<TD><SPAN id='ed_but' class='plain text' onMouseover='do_hover(this.id);' onMouseout='do_plain(this.id);' onClick=\"location.href='../edit.php?id=" + the_requests[key][35] + "';\">Open Ticket</SPAN></TD>";
					} else {
					the_string += "<TD style='white-space: normal; word-wrap: break-word; -ms-word-wrap : sWrap; border-right: 1px solid #707070;'>&nbsp;</TD>";
					}
				the_string += "</TR>";
				}
			the_string += "</TABLE>";
		...
		var reqstbl = document.getElementById('reqstable');
		if(reqstbl) {
			var headerRow = reqstbl.rows[0];
			var viewableRow = 1;
			var headerRow = reqstbl.rows[0];
			for (i = 1; i < reqstbl.rows.length; i++) {
				if(!isViewable(reqstbl.rows[i])) {
					} else {
					viewableRow = i;
					break;
					}
				}
			var tableRow = reqstbl.rows[viewableRow];
			if(tableRow && i != reqstbl.rows.length) {
				...
				var cellwidthBase = document.getElementById('reqstable').clientWidth / 12;
				for (var i = 0; i < tableRow.cells.length; i++) {		
...
function requests_get() {
	if (requests_interval!=null) {return;}
	requests_interval = window.setInterval('do_requests_loop()', 30000);
	}	
