...
@session_write_close();
$ret_arr = array();
$by = (empty($_SESSION)) ? 99999 : $_SESSION['user_id'];
...
if(mysql_affected_rows() > 0) {
	...
	$subject_str3 = "";
	$text_str3 = "";	
	do_log($GLOBALS['LOG_CANCEL_REQUEST'], $by);
	if ($the_email != "") {				// any addresses?
		$to_str1 = $the_email;
		$smsg_to_str1 = "";
		if($row['requester'] == 99999) {
			$subject_str1 = "Your request has been cancelled";
			$text_str1 = "Your Request has been cancelled\n\n"; 
			} else {
	...
	$addrs = notify_newreq($by);		// returns array of adddr's for notification, or FALSE
	if ($addrs) {				// any addresses?
		$to_str2 = implode("|", $addrs);
		$smsg_to_str2 = "";
		$subject_str2 = "Service User request cancelled";
