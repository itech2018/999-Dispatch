...
$al_groups = (array_key_exists('user_groups', $_SESSION)) ? $_SESSION['user_groups'] : array();
...
$result = mysql_query($query);		// 2/10/12
...
$result = mysql_query($query);		// 2/10/12
$the_reqs = (mysql_num_rows($result) > 0) ? mysql_num_rows($result) : "0";

$query = "SELECT * FROM `$GLOBALS[mysql_prefix]requests` WHERE `closed` = '' OR `closed` IS NULL";	//	10/23/12
$result = mysql_query($query);		// 2/10/12
$the_reqs2 = (mysql_num_rows($result) > 0) ? mysql_num_rows($result) : "0";

$query = "SELECT * FROM `$GLOBALS[mysql_prefix]requests` WHERE `danger` = 'Yes' AND `status` = 'Open'";
$result = mysql_query($query);
$the_reqs3 = (mysql_num_rows($result) > 0) ? mysql_num_rows($result) : "0";

$query = "SELECT * FROM `$GLOBALS[mysql_prefix]emer_messages` WHERE `ack_by` = 0";
$result = mysql_query($query);
$emer_help = (mysql_num_rows($result) > 0) ? mysql_num_rows($result) : 0;
$emer_from = array();
$emer_responders = "";
if(mysql_num_rows($result) > 0) {
	...
	$emer_from = array_unique($emer_from);
	$emer_responders = implode(',',$emer_from);
	}
$all_hands = intval(get_variable('all_hands'));
...
/* 	$bits = explode('/', get_variable('status_watch'));
	...
	$query [3] = "SELECT * 
				...
				LEFT KOIN $GLOBALS[mysql_prefix]un_status on $GLOBALS[mysql_prefix]responder.un_status_id = $GLOBALS[mysql_prefix]un_status.id 
				WHERE $GLOBALS[mysql_prefix]un_status.watch = 1 AND `status_updated` < (now() - interval 1 minute)";
...
$the_hash = md5($the_chat_id . $the_tick_id . $the_unit_id . $the_updated . $the_dispatch_change . $the_act_id . $the_pat_id . $the_reqs . $the_reqs2 . $the_reqs3 . $status_updated . $the_status . $status_updated_time . $watch_val . $osw_str . $all_hands . $emer_help . $emer_responders);	//	10/23/12
$ret_arr = array ($the_chat_id, $the_tick_id, $the_unit_id, $the_updated, $the_dispatch_change, $the_act_id, $the_pat_id, $the_reqs, $the_reqs2, $the_reqs3, $status_updated, $the_status, $status_updated_time, $watch_val, $osw_str, $all_hands, $emer_help, $emer_responders, $the_hash);	//	10/23/12
