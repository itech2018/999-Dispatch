...
function get_fieldflag($table, $field) {
	$result = mysql_query("SELECT * FROM `$GLOBALS[mysql_prefix]$table`");
	$field_flags = mysql_field_flags($result, $field);
	return $field_flags;
	}
	
function get_fieldlength($table, $field) {
	$result = mysql_query("SELECT * FROM `$GLOBALS[mysql_prefix]$table`");
	$field_length = mysql_field_len($result, $field);
	return $field_length;
	}
	
function get_fieldindex($table, $name) {
	$table_arr = array();
	$i = 0;
...
function do_caption ($temp, $repl="") { 				// adds a 'captions' table entry - 12/4/10, 01/14/15 added de-dupe function.
	...
	if($num_rows > 1) {	//	if multiple entries exist (an error) delete all where the caption = the replacement value and only leave in custom one
		...
		$result = mysql_query($query) or do_error("", 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
		if($result) { print "Deleted duplicate caption " . $temp . " from Captions table<BR />";}
		$query = "SELECT * FROM `$GLOBALS[mysql_prefix]captions` WHERE `capt` = " . $caption . ";";	// 11/30/10
		$result = mysql_query($query) or do_error("", 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
...
if (!($version == $old_version)) {		// current? - 6/6/2013  ==================================================	
	...
	for ($i=0; $i< count($capts); $i++) {
		...
		$query = "SELECT `capt` FROM `{$the_table}` WHERE `capt` = {$temp} LIMIT 1;";		// Check to see if caption exists
		$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
		if (mysql_num_rows ($result) == 0) {	//	If caption doesn't exist, insert one.
	...
	if(get_fieldlength('settings', 2) < 512) {
...
//					print "Updated status for (open or scheduled) ticket " . $row4['id'] . " in allocates table<BR />";
	...
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` ADD `request_type` INT(4) NOT NULL DEFAULT '0' AFTER `org`;";
	$result = mysql_query($query);
	if($result) { print "Added request type field to requests table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` 
			ADD `water` INT(1) NOT NULL DEFAULT '1' AFTER `ticket_id`, 			
			ADD `power` INT(1) NOT NULL DEFAULT '1' AFTER `water`, 
			ADD `danger` INT(1) NOT NULL DEFAULT '0' AFTER `power`,
			ADD `food` INT(1) NOT NULL DEFAULT '1' AFTER `danger`, 
			ADD `health` INT(1) NOT NULL DEFAULT '0' AFTER `food`,
			ADD `rescue-type` INT(1) NOT NULL DEFAULT '0' AFTER `health`;";
	$result = mysql_query($query);
	if($result) { print "Added water, power, danger and health fields to requests table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` 
			ADD `children` INT(2) NOT NULL DEFAULT '0' AFTER `health`, 
			ADD `adults` INT(2) NOT NULL DEFAULT '0' AFTER `children`, 
			ADD `elderly` INT(2) NOT NULL DEFAULT '0' AFTER `adults`;";
	$result = mysql_query($query);
	if($result) { print "Added children, adults and elderly fields to requests table<BR />";}
	
	if (!table_exists("rescue_type")) {
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]rescue_type` (
			`id` int(2) NOT NULL auto_increment,
			`name` varchar(48) NOT NULL,
			`description` varchar(512) NOT NULL,
			PRIMARY KEY  (`id`)
		...
		$result = mysql_query($query);
		if($result) { print "Added Rescue Type Table<BR />";}
		}
		
	$query = "INSERT INTO `$GLOBALS[mysql_prefix]rescue_type` (`id`, `name`, `description`) VALUES
		(1, 'Wellness Check', 'General check on health and access to essential resources'),		
		(2, 'Water Rescue', 'Boat, Diver or alternative resuce from water or where access is across water'),
		(3, 'Ground Rescue', 'Access across ground, no air or water support required'),
		(4, 'Air Rescue', 'Access restricted, air support required to access from above'),
		(5, 'Medical Rescue', 'Serious health condition, injury or other requires evac to medical facility');";
	$result = mysql_query($query);
	if($result) { print "Inserted Default Rescue Types<BR />";}
	
	do_setting('access_request_url','');
	
	if (!table_exists("stock")) {
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]stock` (
		`id` int(2) NOT NULL auto_increment,
		`name` varchar(48) NOT NULL,
		`description` varchar(512) NOT NULL,
		PRIMARY KEY  (`id`)
		...
		$result = mysql_query($query);
		if($result) { print "Added Stock Table<BR />";}
		}
		
	if (!table_exists("stock_x_facility")) {
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]stock_x_facility` (
		`id` int(8) NOT NULL auto_increment ,
		`facility_id` int(4) NOT NULL ,
		`stock_item` int(4) NOT NULL ,
		`reorder_level` int(8) NOT NULL ,
		`on_order` int(8) NOT NULL ,
		PRIMARY KEY  (`id`)
		) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
		$result = mysql_query($query);
		if($result) { print "Added Stock x facility Table<BR />";}
		}
		
/* 	if (!table_exists("stock_x_facility_x_location")) {
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]stock_x_facility_x_location` (
		`id` int(8) NOT NULL,
		`facility_id` int(4) NOT NULL,
		`stock_item` int(4) NOT NULL,
		`stock_level` int(8) NOT NULL ,
		`location` varchar(48) NOT NULL,
		PRIMARY KEY  (`id`)
		) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
		$result = mysql_query($query);
		if($result) { print "Added Stock x facility x location Table<BR />";}
		} */

	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]facilities` ADD `is_warehouse` INT(1) NOT NULL DEFAULT '0' AFTER `id`;";
	$result = mysql_query($query);
	if($result) { print "Added Is Warehouse field to Facilities table<BR />";}
	
	if (!table_exists("emer_messages")) {		//	For emergency button usage where websockets not usable.
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]emer_messages` (
			`id` int(11) NOT NULL auto_increment,
			`ack_by` int(11) NOT NULL,
			`ack_when` datetime NOT NULL,
			`_by` int(7) NOT NULL,
			`_on` datetime NOT NULL,
			`_from` varchar(16) NOT NULL,
			PRIMARY KEY  (`id`)
		...
		$result = mysql_query($query);
		if($result) { print "Added Emergency Messages Table<BR />";}
		}

	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` 
			ADD `livestock` INT(1) NOT NULL DEFAULT '0' AFTER `elderly`, 
			ADD `livestock_about` TEXT NULL AFTER `livestock`;";
	$result = mysql_query($query);
	if($result) { print "Added Livestock fields to Requests table<BR />";}
	
	$fieldindex = get_field_index('requests', 'water');
	if(get_fieldflag('requests', $fieldindex) != 'not_null enum') {
		$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` 
				 CHANGE `water` `water` ENUM('No','Yes') NOT NULL DEFAULT 'Yes', 
				 CHANGE `power` `power` ENUM('No','Yes') NOT NULL DEFAULT 'Yes', 
				 CHANGE `danger` `danger` ENUM('No','Yes') NOT NULL DEFAULT 'No', 
				 CHANGE `food` `food` ENUM('No','Yes') NOT NULL DEFAULT 'Yes',
				 CHANGE `livestock` `livestock` ENUM('No','Yes') NULL DEFAULT 'No',
				 CHANGE `health` `health` ENUM('No','Yes', 'Unknown') NOT NULL DEFAULT 'No';";
		$result = mysql_query($query);
		if($result) { print "Revised various request fields to ENUM type in Requests table<BR />";}
		}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]requests` CHANGE `rescue-type` `rescue_type` INT(1) NOT NULL DEFAULT '0';";
	$result = mysql_query($query);
	if($result) { print "Changed name of rescue-type field to rescue_type in Requests table<BR />";}

	$query = "DROP TABLE `$GLOBALS[mysql_prefix]stock_x_facility_x_location`;";
	$result = mysql_query($query);
	if($result) { print "Dropped stock_x_facility_x_location table as no longer required<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]stock` 
			ADD `order_quantity` INT(8) DEFAULT NULL AFTER `description`,
			ADD `pack_size` INT(8) DEFAULT NULL AFTER `order_quantity`, 
			ADD `reorder_level` INT(8) DEFAULT NULL AFTER `pack_size`;";
	$result = mysql_query($query);
	if($result) { print "Added order_quantity, pack_size and reorder_level to Stock table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]stock_x_facility` DROP `reorder_level` ";
	$result = mysql_query($query);
	if($result) { print "Dropped reorder_level field from stock_x_facility table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]stock_x_facility` ADD `stock_level` VARCHAR(48) DEFAULT NULL AFTER `on_order`";
	$result = mysql_query($query);
	if($result) { print "Added Stock Level to stock_x_facility table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]stock_x_facility` ADD `location` INT(8) DEFAULT NULL AFTER `stock_level`";
	$result = mysql_query($query);
	if($result) { print "Added Location to stock_x_facility table<BR />";}
	
	do_setting('all_hands','0');
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]organisations` ADD `logo` VARCHAR(128) NULL DEFAULT NULL AFTER `email`";
	$result = mysql_query($query);
	if($result) { print "Added Logo field to organisations table<BR />";}

	if(get_fieldlength('settings', 2) < 1024) {
		$query = "ALTER TABLE `$GLOBALS[mysql_prefix]settings` CHANGE `value` `value` VARCHAR( 1024 ) CHARACTER SET latin1 COLLATE latin1_swedish_ci NULL DEFAULT NULL ;";
		$result = mysql_query($query);
		if($result) { print "Altered settings table value field to 1024 characters<BR />";}
		}

	do_mdb_setting ('contact_list_fields','1,2,3,4,5,7,8,22,23,24,25');
	do_setting('login_warning','');

	if (!table_exists("emer_messages")) {		//	For emergency button usage where websockets not usable.
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]patient_obs` (
			`id` int(16) NOT NULL auto_increment,
			`patient_id` int(12) NOT NULL,
			`blood_pressure` varchar(24) DEFAULT NULL,
			`heart_rate` varchar(24) DEFAULT NULL,
			`core_temp` varchar(24) DEFAULT NULL,
			`o2_sats` varchar(24) DEFAULT NULL,
			`gcs` varchar(24) DEFAULT NULL,
			`symptoms` text,
			`notes` text,
			`_by` int(7) NOT NULL,
			`_on` datetime NOT NULL,
		...
		$result = mysql_query($query);
		if($result) { print "Added Patuent OBS Table<BR />";}
		}

	$query = "INSERT INTO `$GLOBALS[mysql_prefix]sound_settings` (`id`, `name`, `filename`, `mp3_filename`, `ison`) VALUES
		(6, 'man_down', 'aooga.wav', 'klaxon.mp3', 1);";
	$result = mysql_query($query);
	if($result) { print "Inserted 'Man Down' Sound<BR />";}

	if (!table_exists("zello_channels")) {
		$query = "CREATE TABLE IF NOT EXISTS `$GLOBALS[mysql_prefix]zello_channels` (
			`id` int(2) NOT NULL,
			`name` varchar(64) NOT NULL,
		...
		$result = mysql_query($query);
		if($result) { print "Added Zello Channels Table<BR />";}
		}
		
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]user` ADD `zello_user` VARCHAR(48) NULL DEFAULT NULL AFTER `sid`";
	$result = mysql_query($query);
	if($result) { print "Added Zello User to User table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]user` ADD `zello_password` VARCHAR(48) NULL DEFAULT NULL AFTER `zello_user`";
	$result = mysql_query($query);
	if($result) { print "Added Zello Password to User table<BR />";}
	
	$query = "ALTER TABLE `$GLOBALS[mysql_prefix]user` ADD `zello_channel` VARCHAR(128) NULL DEFAULT NULL AFTER `zello_password`";
	$result = mysql_query($query);
	if($result) { print "Added Zello User Channel to User table<BR />";}
	
	do_setting('use_zello','0');
	
	$now = mysql_format_date(time());
	$where = $_SERVER['REMOTE_ADDR'];
	
	if(table_exists('fac_types') && get_table_rows('fac_types') == 0) {
		$query = "INSERT INTO `$GLOBALS[mysql_prefix]fac_types` (`name`, `description`, `icon`, `_by`, `_from`, `_on`) VALUES
			('example', 'example', 0, 1, '" . $where . "', '" . $now . "');";
		$result = mysql_query($query);
		if($result) { print "Added Example Facility Types<BR />";}
		}

	if(table_exists('fac_status') && get_table_rows('fac_status') == 0) {		
		$query = "INSERT INTO `$GLOBALS[mysql_prefix]fac_status` (`status_val`, `description`, `group`, `status_available`, `status_unavailable`, `sort`, `bg_color`, `text_color`, `_by`, `_from`, `_on`) VALUES
				('example_available', 'example_available', 'Available', 0, 0, 1, '#1BFF1B', '#000000', 1, '" . $where . "', '" . $now . "'),
				('example_unavailable', 'example_unavailable', 'Unavailable', 0, 0, 1, '#FF2424', '#000000', 1, '" . $where . "', '" . $now . "');
				";
		$result = mysql_query($query);
		if($result) { print "Added Example Facility Statuses<BR />";}
		}
