...
$GLOBALS['LOC_TYPES_NAMES']	= array('Violence','Frequent','Health','Environmental','General','Unstable Structure','Building Evacuated');
$GLOBALS['LOC_TYPES'] = array(0,1,2,3,4,5,6);		//	11/10/14
$GLOBALS['LOC_TYPES_BG']	= array('#FF0000','#000000','#FFFFFF','#FFFF00','#0000FF','FF00FF','0000FF');		//	11/10/14
$GLOBALS['LOC_TYPES_TEXT']	= array('#FFFFFF','#FFFFFF','#000000','#000000','#FFFFFF','#000000','#000000');		//	11/10/14
$GLOBALS['wl_icons'] = array("square_red.png", "square_black.png", "square_white.png", "square_yellow.png", "square_blue.png", "square_green.png", "square_bronze.png");
$GLOBALS['wl_sm_icons']	= array("sm_square_red.png", "sm_square_black.png", "sm_square_white.png", "sm_square_yellow.png", "sm_square_blue.png", "sm_square_green.png", "sm_square_bronze.png");
...
function format_date_RFC850($date) {
	$date_wk = (strlen(trim($date))== 19)? strtotime(trim($date)) : trim($date) ;			// force to integer
	return date(DATE_RFC850, intval($date_wk));
	}
...
function get_zello($id){
	$query = "SELECT `zello_user`, `zello_password`, `zello_channel` FROM `$GLOBALS[mysql_prefix]user` WHERE `id`= " . $id . " LIMIT 1";
	$result	= mysql_query($query);
	$ret_arr = array();
	if($result && mysql_num_rows($result) == 1) {
		$row = stripslashes_deep(mysql_fetch_assoc($result));
		$ret_arr['user'] = $row['zello_user'];
		$ret_arr['password'] = $row['zello_password'];
		$ret_arr['channel'] = $row['zello_channel'];
		return $ret_arr;
		} else {
		return false;
		}
...
function username_from_name($name){								/* get owner id from name */
	$query = "SELECT * FROM `$GLOBALS[mysql_prefix]user` WHERE `user`= '$name' LIMIT 1";
	$result	= mysql_query($query);
	if($result) {
		$row = stripslashes_deep(mysql_fetch_assoc($result));
		return (mysql_affected_rows()==0 )? 0 : $row['id'];
		} else {
		return 0;
		}
	}
...
function is_manager(){					/* is user team manager? */			
...
function generate_date_dropdown($date_suffix,$default_date=0, $disabled=FALSE) {			// 'extra allows 'disabled'
	...
	switch($locale) { 
		case "0":
			print "<SELECT class='text' id='year_$date_suffix' name='frm_year_$date_suffix' $dis_str>";
			...
			print "</SELECT>";
			print "&nbsp;<SELECT class='text' id='month_$date_suffix' name='frm_month_$date_suffix' $dis_str>";
			for($i = 1; $i < 13; $i++){
				print "<OPTION class='text' VALUE='$i'";
				$month == $i ? print " SELECTED>$i</OPTION>" : print ">$i</OPTION>";
				}
			
			print "</SELECT>\n&nbsp;<SELECT class='text' id='day_$date_suffix' name='frm_day_$date_suffix' $dis_str>";
			for($i = 1; $i < 32; $i++){
				print "<OPTION class='text' VALUE=\"$i\"";
		...
		case "1":
			print "<SELECT class='text' id='day_$date_suffix' name='frm_day_$date_suffix' $dis_str>";
			for($i = 1; $i < 32; $i++){
				print "<OPTION class='text' VALUE=\"$i\"";
				$day == $i ? print " SELECTED>$i</OPTION>" : print ">$i</OPTION>";
				}
	...
	print "\n<INPUT class='text' TYPE='text' SIZE='2' MAXLENGTH='2' id='hour_$date_suffix' NAME='frm_hour_$date_suffix' VALUE='$hour' $dis_str>:";
	print "\n<INPUT class='text' TYPE='text' SIZE='2' MAXLENGTH='2' id='minute_$date_suffix' NAME='frm_minute_$date_suffix' VALUE='$minute' $dis_str>";
	$show_ampm = (!get_variable('military_time')==1);
	if ($show_ampm){	//put am/pm optionlist if not military time
		print "\n<SELECT class='text' id='meridiem_$date_suffix' NAME='frm_meridiem_$date_suffix' $dis_str><OPTION class='text' value='am'";
...
function shorten($instring, $limit) {
	if(is_array($instring) && array_key_exists(0, $instring)) {
		$theString = $instring[0];
		} else {
		$theString = $instring;
		}
	return (strlen($theString) > $limit)? substr($theString, 0, $limit-4) . ".." : $theString ;	// &#133
	}
...
function do_log($code, $ticket_id=0, $responder_id=0, $info="", $facility_id=0, $rec_facility_id=0, $memberid=0, $mileage=0) {		// generic log table writer - 5/31/08, 10/6/09
	@session_start();							// 4/4/10
	$who = (array_key_exists('user_id', $_SESSION))? $_SESSION['user_id']: 0;		// 11/14/10
	$info = substr($info, 0, 2047);
	$from = $_SERVER['REMOTE_ADDR'];
	$now = mysql_format_date(time() - (intval(get_variable('delta_mins'))*60));
	$query = sprintf("INSERT INTO `$GLOBALS[mysql_prefix]log` (`who`,`from`,`when`,`code`,`ticket_id`,`responder_id`,`member_id`,`info`, `facility`, `rec_facility`, `mileage`)  
...
function do_log_failed_login($code, $ticket_id=0, $responder_id=0, $info="", $facility_id=0, $rec_facility_id=0, $memberid=0, $mileage=0, $username) {		// to log failed login attempts
	@session_start();							// 4/4/10
	$who = username_from_name($username);
	$info = substr($info, 0, 2047);
	$from = $_SERVER['REMOTE_ADDR'];
	$now = mysql_format_date(time() - (intval(get_variable('delta_mins'))*60));
	$query = sprintf("INSERT INTO `$GLOBALS[mysql_prefix]log` (`who`,`from`,`when`,`code`,`ticket_id`,`responder_id`,`member_id`,`info`, `facility`, `rec_facility`, `mileage`)  
...
//	print $to_str . "," . $smsg_to_str . "," . $subject_str . "," . $text_str . "," . $ticket_id . "," . $responder_ids . "<BR />";
	$fromname = get_variable("email_from");
	$the_resp_ids = "";
	if($responder_ids != 0) {
		...
		if(count($to_array) > 0) {
			...
			if (count($ary_ll_addrs)>0) {												// got landline addee's?
				...
				if (count($my_smtp_ary)>1) {
					...
					store_email(1, $the_responders, "email", $subject_str, $text_str, $ticket_id, $the_resp_ids, date("Y/m/d H:i:s", $now), $my_replyto_str, $fromname);	// 7/9/12	
					} else {
		//								($my_smtp_ary, $my_to_ary, $my_subject_str, $my_message_str, $my_from_ary, $my_replyto_str)
...
function notify_newreq($svceuser_id, $state = null) {								// 10/23/12
	...
	if(!$state) {
		...
		$query = "SELECT * FROM `$GLOBALS[mysql_prefix]user` WHERE (`level` = '0' OR `level` = '1') AND (`addr_st` = '" . $state . "')";	//	Get all users admin and super that have valid email address stored and save for emailing.
		}			
	$result	= mysql_query($query) or do_error($query,'mysql_query() failed',mysql_error(), basename( __FILE__), __LINE__);
	while($row = stripslashes_deep(mysql_fetch_assoc($result))) {
...
function get_localuser($state) {
	$users = array();
	$query = "SELECT * FROM `$GLOBALS[mysql_prefix]user` WHERE (`level` = '0' OR `level` = '1' OR `level` = '2') AND (`addr_st` = '" . $state . "')";	//	Get all users admin and super that have valid email address stored and save for emailing.
	$result	= mysql_query($query) or do_error($query,'mysql_query() failed',mysql_error(), basename( __FILE__), __LINE__);
	while($row = stripslashes_deep(mysql_fetch_assoc($result))) {
		$users[] = $row['id'];
		}
...
function get_stock_name($id) {
	$query = "SELECT * FROM `$GLOBALS[mysql_prefix]stock` WHERE `id` = " . $id;
	$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
	if(mysql_num_rows($result) > 0) {
		$row = stripslashes_deep(mysql_fetch_assoc($result));
		$theName = $row['name'];
...
function get_units_legend() {		// returns string as centered span - 2/8/10
	...
	$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
	$out_str = "<SPAN CLASS = 'text' ALIGN = 'center'><SPAN CLASS = 'text text_bold' ALIGN = 'center'>Unit Types: </SPAN>&nbsp;";
	while ($row = stripslashes_deep(mysql_fetch_assoc($result))) {
...
*/
	...
	function format_sb_date_4($date_in){
		return date("j/n H:i", intval($date_in));	//	Day and Hour
		}
...
function add_sidebar($regions = TRUE, $files = TRUE, $messages = TRUE, $controls = TRUE, $more=FALSE, $allowedit=FALSE, $ticket_id = 0, $responder_id = 0, $facility_id = 0, $mi_id = 0) {
	...
	$zello = intval(get_variable('use_zello'));
	if($zello == 1) {
		$print .= "<SPAN id='z_ct' class='plain text' TITLE='Zello' style='position: fixed; top: 380px; right: 0px; width: 55px; display: inline-block; cursor: pointer; padding: 2px; background-color: #FEFEFE;' 
			onMouseOver='do_hover(this.id);' onMouseOut='do_plain(this.id);' 
			onClick=\"zello_window();\">
			<IMG src='./buttons/zello.png' ALT='Zello PTT'></SPAN>";
		}
	$print .= "</DIV>";
	return $print;
	}
...
function show_direc($count = 20) {
	...
	$accessTokenSecret = get_variable('twitter_accesstokensecret');
	$twitter = new Twitter($consumerKey, $consumerSecret, $accessToken, $accessTokenSecret);
	$messages = $twitter->request('direct_messages/events/list', 'GET', ['count' => 20]);
	$count = count($messages->events);
	if($count > 0) {
		$print = "";
		$print .= "<ul>";
