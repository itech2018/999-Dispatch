...
javAPRSSrvr - javAPRSSrvr must be accessible from the server Tickets is on.
              ...
              Presently uses server, database, user, and password for xastir configuration.
4/4/2019 - corrections applied to followmee; resuorce control loop and api call string -- AS
*/
...
//			snap (__LINE__, count($data));
			if (count($data) > 0){
				...
				if ( sane ( floatval ($lat), floatval ($lng), intval (strtotime($updated)) ) && ( is_recent($packet_date) ) ) {
					...
					$result_2 = @mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__); //11/15/11
//					snap (__LINE__, $query);
					if ( mysql_affected_rows() > 0 ) {									// movement ?
...
//						snap (__LINE__, $query);
						$result_2 = @mysql_query($query);		// 6/17/2015
						}

					}			// end if (sane( ... ))
				}			// end ( JSON data OK)
			}			// end while ($row = @mysql_fetch_assoc()
