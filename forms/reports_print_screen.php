...
?>
<SCRIPT>
var org_logos = '<?php echo json_encode($orgLogos);?>';
...
var outerwidth;
var currDate = "<?php print $currDate;?>";
var organisation = "<?php print $organisation;?>";
...
function goGetit(func, what) {
	...
	$('report3').innerHTML = "";
	if(func == "ugr") {
		var url = "./ajax/reports.php?report=" + which + "&func=" + what + "&date=" + currDate + "&tick_sel=" + ticksel + "&resp_sel=" + respsel + "&startdate=" + startdate + "&enddate=" + enddate + "&width=100%&organisation=" + organisation + "&do_print=1&version=" + randomnumber;
		} else {
		var url = "./ajax/reports.php?report=" + which + "&func=dr&date=" + what + "&tick_sel=" + ticksel + "&resp_sel=" + respsel + "&startdate=" + startdate + "&enddate=" + enddate + "&width=100%&organisation=" + organisation + "&do_print=1&version=" + randomnumber;
		}
	...
	function reports_cb(req) {
		...
		setTimeout(function() {
			if(theResponse[0]) {
			var theOrgLogos = JSON.decode(org_logos);
			if(which == "b") {
				if(theOrgLogos[organisation]) {
					$('report_header').innerHTML = "<IMG id='head_img' style='display: block; vertical-align: middle; float: left;' src=\"" + theOrgLogos[organisation] + "\"/>";
					} else {
					$('report_header').innerHTML = "<IMG id='head_img' style='display: block; vertical-align: middle; float: left;' src=\"<?php print get_variable('report_graphic');?>\"/>";
					}
				} else {
				$('report_header').innerHTML = "<IMG id='head_img' style='display: block; vertical-align: middle; float: left;' src=\"<?php print get_variable('report_graphic');?>\"/>";
				}
		...
		setTimeout(function() {
			$('report_header').style.height = $('head_img').clientHeight + "px";
			$('report_table_head').style.height = $('head_img').clientHeight + "px";
			}, 500);
		}
	}
...
<DIV id='outer' style='position: absolute; left: 0px; z-index: 1;'>
	<DIV id='leftcol' style='position: absolute; left: 5px; top: 10px; z-index: 3; width: 98%;'>
		...
		<DIV id='report_table_head' class='heading text_large text_center' style='position: relative; top: 60px; width: 100%; display: block; height: auto; padding-top: 5px; padding-bottom: 5px;'>
			<SPAN id='report_header' style='text-align: center; display: block; height: auto;'></SPAN>
		</DIV>
		<BR /><BR />
