...
function do_statistics() {
	...
	function stats_cb(req) {
		...
		if(theStats.length == "undefined" || theStats.length != 12) {return;}
...
function set_size() {
	...
	listHeight = viewportheight * .20;
	innerListHeight = viewportheight * .17;
	...
	$('the_list').style.maxHeight = innerListHeight + "px";
	...
	$('the_rlist').style.maxHeight = innerListHeight + "px";
	...
	$('the_flist').style.maxHeight = innerListHeight + "px";
...
?>	
	try { parent.frames["upper"].$("personnel").style.display = "none";}	
	catch(e) { }
<?php
	}		// end not admin or super
...
<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
	<DIV CLASS='header' style = "height:32px; width: 100%; float: none; text-align: center;">
		<A id='maj_incs' class='plainmi text_bold text_large' style='display: none;' onMouseover='do_hover_mi(this.id);' onMouseout='do_plain_mi(this.id);' HREF="maj_inc.php"></A>
...
if($('the_list')) {$('the_list').style.maxHeight = innerListHeight + "px";}
if($('the_list')) {$('the_list').style.width = listwidth + "px";}
