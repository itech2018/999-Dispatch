...
$manager_team = (count(manager_team($user_id)) > 0) ? manager_team($user_id) : FALSE ;
...
function member_list() {
	global $istest, $internet, $num_rows, $manager_team, $level, $user_id;
...
//	Major While
	if($num_rows == 0) {
		...
		while ($row = stripslashes_deep(mysql_fetch_assoc($result))) {
			$teams_managed = manager_team($user_id);
			foreach($teams_managed AS $val) {
				if(($level == $GLOBALS['LEVEL_MANAGER'] && $val == $row['team']) || $level == $GLOBALS['LEVEL_SUPER'] || $level == $GLOBALS['LEVEL_ADMINISTRATOR']) {
					if((can_edit()) || (is_curr_member($row['member_id']))) {
