...
if($num_locations == 0) {
	...
	while ($row = stripslashes_deep(mysql_fetch_assoc($result))) {		// ==========  major while() for Location ==========
		...
		if(is_guest() || $screen == "fullscreen") {
			...
			$toedit = "<SPAN id='editbut' CLASS='plain text' style='float: none;' onMouseover='do_hover(this.id);' onMouseout='do_plain(this.id);' onClick='locEdit(" . $row['id'] . ");'>Edit</SPAN>" ;
			$toview = "<SPAN id='viewbut' CLASS='plain text' style='float: none;' onMouseover='do_hover(this.id);' onMouseout='do_plain(this.id);' onClick='locView(" . $row['id'] . ");'>View</SPAN>";
			}	
