...
$allmessages = (isset($_GET['allmess']) && $_GET['allmess'] == "yes") ? true : false;
	...
	while ($msg_row = stripslashes_deep(mysql_fetch_assoc($result))){
		...
		if($msg_row['recipients'] == NULL) {
			...
			if($msg_row['recipients'] == "Tickets") {
				$respstring = get_variable("email_reply_to");
				} else {
		...
		$fromname = ($msg_row['fromname'] != "") ? shorten($msg_row['fromname'], 30) : shorten($msg_row['from_address'], 30);
		$fromname = ($fromname == "") ? "TBA" : $fromname;
		$ret_arr[$i][0] = $the_message_id;		
		$ret_arr[$i][1] = $msg_row['ticket_id'];
		$ret_arr[$i][2] = $type_flag;
		$ret_arr[$i][3] = shorten($fromname, 18);
		$ret_arr[$i][4] = shorten($respstring, 20);
