...
if (mysql_num_rows($result) == 0) {
	$ret_arr[0][0] = "No Assignments Currently";
	} else {
	$i = 0;
	while ($row = stripslashes_deep(mysql_fetch_assoc($result))){	
		$query2 = "SELECT * FROM `$GLOBALS[mysql_prefix]ticket` WHERE `id` = '" . $row['ticket_id'] . "' AND `status` = " . $GLOBALS['STATUS_OPEN'] . " LIMIT 1"; 		
		$result2 = mysql_query($query2);
		if($result2 && mysql_num_rows($result2) == 1) {
			$row2 = stripslashes_deep(mysql_fetch_assoc($result2));
			if(array_key_exists('id', $row2) && array_key_exists('scope', $row2) && array_key_exists('lat', $row2) && array_key_exists('lng', $row2)) {
				$ret_arr[$i][0] = $row2['id'];
