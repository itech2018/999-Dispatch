...
*/
	...
	function do_sel_update_fac(in_unit, in_val, theIcon) {
		to_server_fac(in_unit, in_val, theIcon);
		}
		
	function to_server_fac(the_unit, the_status, theIcon) {
		var querystr = "frm_responder_id=" + the_unit;
		querystr += "&frm_status_id=" + the_status;
		var url = "as_up_fac_status.php?" + querystr;
		sendRequest (url,svrfacCB, "");
		function svrfacCB(req) {
			var payload = req.responseText;
			if (payload.substring(0,1)=="-") {	
				alert ("<?php print __LINE__;?>: msg failed ");
				return false;
	...
	function do_sel_update_mem(in_member, in_val) {
		to_server_mem(in_member, in_val);
		}

	function to_server_mem(the_member, the_status) {
		var querystr = "frm_member_id=" + the_member;
		querystr += "&frm_status_id=" + the_status;
		var url = "as_up_mem_status.php?" + querystr;
		sendRequest (url,svrmemCB, "");
		function svrmemCB(req) {
			var payload = req.responseText;
			if (payload.substring(0,1)=="-") {	
				alert ("<php print __LINE__;?>: msg failed ");
				return false;
				} else {
				parent.frames['upper'].show_msg ('Member status update applied!')
				return true;
				}
			}
		}
		
	function do_sel_update_type (in_member, in_val) {
		to_server2(in_member, in_val);
		}
	...
	function to_server2(the_member, the_type) {							// write Member status data via ajax xfer
		var querystr = "frm_member_id=" + the_member;
		querystr += "&frm_type_id=" + the_type;
		var url = "as_up_mem_type.php?" + querystr;	
		sendRequest (url,svr2CB, "");
		function svr2CB(req) {
			var payload = req.responseText;
			if (payload.substring(0,1)=="-") {	
				alert ("<php print __LINE__;?>: msg failed ");
				...
				parent.frames['upper'].show_msg ('Member type update applied!')
				return true;
				}				// end if/else (payload.substring
			}
		}
...
function do_hover_mi (the_id) {
	CngClass(the_id, 'hovermi text_bold text_large');
	return true;
	}

function do_plain_mi (the_id) {				// 8/21/10
	CngClass(the_id, 'plainmi text_bold text_large');
	return true;
	}
...
function zello_window() {
	var url = "zello.php";
	var zelloWindow = window.open(url, 'zelloWindow', 'resizable=1, scrollbars, height=600, width=1000, left=100,top=100,screenX=100,screenY=100');
	setTimeout(function() { zelloWindow.focus(); }, 1);
	}
...
function set_fontsizes(screenwidth, screenFormat) {
	var theFonts = {verylarge:"18px",large:"16px",normal:"14px",medium:"12px",small:"10px",verysmall:"8px",massive:"22px"};
	var theScreen = ($('screenname')) ? $('screenname').innerHTML : "undefined";
	var listPadding = "1.5em";
	var theScreenSize;
	if(screenFormat == "popup") {
		if(screenwidth > 1000) {
			theFonts.massive = "20px";
	...
	jss.set('.text_massive', { "font-size": theFonts.massive });
...
function locView(id) {
	document.view_loc_Form.id.value=id;			// 10/16/08, 10/25/08
	document.view_loc_Form.submit();		
	}
	
function locEdit(id) {
	document.edit_loc_Form.id.value=id;			// 10/16/08, 10/25/08
	document.edit_loc_Form.submit();		
	}
