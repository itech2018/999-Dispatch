...
function set_fac_categories() {
	...
	for (var i = 0; i < fac_curr_cats.length; i++) {
		...
		if(fac_cat_sess_stat[i]=="s") {
			...
			if($(fac_catname)) {$(fac_catname).checked = true;}
			} else {
			for (var j = 0; j < fmarkers.length; j++) {
				if((fmarkers[j]) && (fmarkers[j].category == fac_catname)) {
			...
			if($(fac_catname)) {$(fac_catname).checked = false;}
			}				
		}
	if(factbl) {
...
function pt_to_map (my_form, lat, lng) {
	...
	if(document.wiz_add) {
		if(document.wiz_add.wiz_show_lat) {document.wiz_add.wiz_show_lat.value=do_lat_fmt(theLat);}
		if(document.wiz_add.wiz_show_lng) {document.wiz_add.wiz_show_lng.value=do_lng_fmt(theLng);}
		if(locale == 0) { 
			if(document.wiz_add.wiz_frm_ngs) {document.wiz_add.wiz_frm_ngs.value=LLtoUSNG(theLat, theLng, 5);}
			}
		if(locale == 1) {
			if(document.wiz_add.wiz_frm_ngs) {document.wiz_add.wiz_frm_ngs.value=LLtoOSGB(theLat, theLng, 5);}
			}
		if(locale == 2) {
			if(document.wiz_add.wiz_frm_ngs) {document.wiz_add.wiz_frm_ngs.value=LLtoUTM(theLat, theLng, 5);}
			}		
		}
...
function all_users_all_groups() {
	var url = "set_all_hands.php";
	sendRequest (url, alag_handleResult, "");
	}
	
function alag_handleResult(req) {
	var response = JSON.decode(req.responseText);
	if(response[0] == 1) {
		if($('alag_but')) {$('alag_but').innerHTML = "All Hands Off";}
		alert("All Hands set to ON");
		} else {
		if($('alag_but')) {$('alag_but').innerHTML = "All Hands On";}
		alert("All Hands set to OFF");
		}
	}
...
function load_poly_controls() {
	...
	function polys_cb(req) {
		...
		if($('poly_boxes')) {$('poly_boxes').innerHTML = outputtext;}
...
function get_theMessages(ticket_id, responder_id, facility_id, mi_id, sort, dir, inorout) {
	...
	var randomnumber=Math.floor(Math.random()*99999999);
	var url ='./ajax/sidebar_list_messages.php'+theSearchstring+theSortField+theOrder+"&version=" + randomnumber + "&inorout=" + window.inorout+'&allmess=yes&q='+sess_id;
...
function doTweet(myForm) {
	...
	if(tUserid == "" && tScreenname == ""){
		...
		var url = './ajax/twitter_direct_send.php?message=' + tMessage + tUserid + tScreenname + '&version=' + randomnumber + '&q=' + sess_id;		
		}
	sendRequest (url, theCB, "");
	function theCB(req) {
...
function get_assigns(unit_id) {
	...
	if(resp_assigns[unit_id] != "undefined") {
		if(num_ass == 1) {
